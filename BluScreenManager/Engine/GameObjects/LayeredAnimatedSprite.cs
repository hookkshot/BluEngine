...
namespace BluEngine.Engine.GameObjects
{
    public class LayeredAnimatedSprite : AnimatedSprite
    {
        ...
        public override void Draw(SpriteBatch spriteBatch,Vector2 screenOffset)
        {
            foreach (Sprite sprite in sourceImages)
            {
                spriteBatch.Draw(sprite.SourceImage, ConnectedGameObject.Position - screenOffset, new Rectangle(frameWidth * currentFrame, 0, frameWidth, frameHeight), Color.White, ConnectedGameObject.Rotation, new Vector2(-frameWidth / 2, -frameHeight / 2), ConnectedGameObject.Scale, SpriteEffects.None, sprite.Layer);
            }
        }
    }
}
